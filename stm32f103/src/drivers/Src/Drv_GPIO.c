...
void GPIO_Init(GPIO_Handle_t* pGPIOHandle)
{
    ...
    pReg = (pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber < GPIO_PIN_8) ? &pGPIOHandle->pGPIOx->CRL : &pGPIOHandle->pGPIOx->CRH;
    ...
    *pReg |= (temp << ((pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber & 0x07) * 4));
    ...
    if (temp == GPIO_MODE_OUTPUT_PP || temp == GPIO_MODE_OUTPUT_OD || temp == GPIO_MODE_AF_PP || temp == GPIO_MODE_AF_OD)
    {
    ...
    {
        pReg = (temp == GPIO_PUPD_PULL_UP) ? &pGPIOHandle->pGPIOx->BSRR : &pGPIOHandle->pGPIOx->BRR;
        *pReg = (1UL << pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber);
    }

    // Configure alternate function (AF) for certain modes
    temp = (uint32_t)pGPIOHandle->GPIO_PinConfig.GPIO_PinMode;
    if (temp == GPIO_MODE_AF_PP || temp == GPIO_MODE_AF_OD || temp == GPIO_MODE_IT_RISING || temp == GPIO_MODE_IT_FALLING ||
        temp == GPIO_MODE_IT_RISING_FALLING)
    {
        AFIO_CLOCK_ENABLE();

        if (pGPIOHandle->pGPIOx == GPIOA)
            temp = AFIO_EXTICR_PORT_PA;
        else if (pGPIOHandle->pGPIOx == GPIOB)
            temp = AFIO_EXTICR_PORT_PB;
        else if (pGPIOHandle->pGPIOx == GPIOC)
            temp = AFIO_EXTICR_PORT_PC;
        else if (pGPIOHandle->pGPIOx == GPIOD)
            temp = AFIO_EXTICR_PORT_PD;
        else if (pGPIOHandle->pGPIOx == GPIOE)
            temp = AFIO_EXTICR_PORT_PE;
        else if (pGPIOHandle->pGPIOx == GPIOF)
            temp = AFIO_EXTICR_PORT_PF;
        else if (pGPIOHandle->pGPIOx == GPIOG)
            temp = AFIO_EXTICR_PORT_PG;
